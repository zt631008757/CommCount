...
public class ImageUtil {
    ...
    public static File saveImage(Context context, Bitmap bmp) {
        // 首先保存图片
        String storePath = FileUtil.getDiskCacheRootDir(context) + File.separator + "image";
...
//        String storePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "image";
        String storePath = context.getExternalFilesDir(Environment.DIRECTORY_PICTURES) + File.separator + "image";
        File appDir = new File(storePath);
        if (!appDir.exists()) {
            appDir.mkdir();
        }
